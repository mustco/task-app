// app/components/tasks/edit-task-dialog.tsx
"use client";
...
export function EditTaskDialog({
  ...
  const [remindMethod, setRemindMethod] =
    useState<Task["remind_method"]>("whatsapp");
  ...
  useEffect(() => {
    ...
    if (taskToEdit.deadline) {
      ...
      setDeadline(new Date(d.getTime() - offset).toISOString().slice(0, 16));
    } else {
      setDeadline("");
    }

    // kalau task lama tidak punya reminder, pakai default yang wajar
    const method =
      (taskToEdit.remind_method as Task["remind_method"]) || "whatsapp";
    setRemindMethod(method);
    setReminderDays(taskToEdit.reminder_days ?? 0);

    if (method !== "email") setWhatsappContact(taskToEdit.target_phone || "");
    else setWhatsappContact("");

    if (method !== "whatsapp") setEmailContact(taskToEdit.target_email || "");
    else setEmailContact("");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {/* 📱 mobile friendly: width adaptif + body scroll */}
